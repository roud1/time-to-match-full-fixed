...
import { emitActivityBump } from "@/client/lib/activity-notify"
import { broadcastPresenceUpdate } from "@/client/lib/presence/realtime-presence"
import { getPulseProfile } from "@/client/lib/pulse/profile"
import { isPulseProfileId } from "@/client/lib/pulse/constants"
import { getAppMode } from "@/client/lib/auth/client"
import { postDiscoverLike, postDiscoverPass } from "@/client/lib/discover/api"
import { discoverIdToNumeric } from "@/client/lib/discover/map-profile"
import { fetchMatchDetail, sendMatchMessage, type ServerMatchMessage } from "@/client/lib/matches/api"
import {
  chatThreadUpdatedAt,
  mergeChatThreads,
  serverMessagesToChat,
  threadsFromServerMatches,
} from "@/client/lib/matches/map-chat"
import { resolveMatchIdForProfile, storeServerMatchId } from "@/client/lib/matches/resolve"
...
function finalizeSwipeSideEffects(
  ...
  if (matched && direction === "right") {
    ...
    void import("@/client/lib/gamification/api").then(({ reportAchievementEvent, dispatchGamificationUpdate }) =>
...
export async function loadServerChatThreads(
  ...
  const { fetchActiveMatches } = await import("@/client/lib/match-freeze-client")
