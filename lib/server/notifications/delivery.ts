import { buildNotificationContent } from "@/lib/server/notifications/copy"
import { markNotificationSent } from "@/lib/server/notifications/repository"
import type { PendingNotificationRow } from "@/lib/server/notifications/repository"
import { sendEmail } from "@/lib/server/email/send-email"
import { sendWebPush } from "@/lib/server/push/web-push"
...
export async function sendPendingNotifications(): Promise<{
  ...
  const { fetchPendingForDelivery } = await import("@/lib/server/notifications/repository")
