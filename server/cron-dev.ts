...
export function startDevNotificationsCron() {
  ...
  const tick = async () => {
    try {
      const { runNotificationsCron } = await import("@/server/notifications/run-cron")
...
export function startDevExpireMatchesCron() {
  ...
  const tick = async () => {
    try {
      const { runEnginesCron } = await import("@/server/engines/expiration/expiration.worker")
...
export function startDevAiAnalysisCron() {
  ...
  const tick = async () => {
    try {
      const { processConnectionAnalysisJobs } = await import("@/server/connection-ai-processor")
